...
- [ ] Create `scripts/check_model_drift.py`: CLI wrapper around `drift_detector.check_drift()`, prints JSON report, exits 1 if status == "fail"
- [ ] Add `make check-drift` Makefile target
- [ ] Add a **Model Health** section to the Streamlit admin sidebar (`dashboard/app.py`):
...
- [ ] Add `analytics.drift_log` ClickHouse table (`infra/clickhouse/sql/008_drift_log.sql`):
...
- [ ] Add `redpandadata/console` service to `docker-compose.yml` (port 8080):
...
- [ ] Add slow query panel to Streamlit admin sidebar:
...
- [ ] Add `make console-up` Makefile target: `docker compose up -d redpanda-console`
...
- [ ] Create `.github/workflows/ci.yml`:
  ...
  ```
- [ ] Ensure all existing tests pass under `pytest tests/` (fix any broken imports from new modules added in Phases 18-20)
- [ ] Add `make lint` Makefile target: `ruff check src/ scripts/ tests/`
- [ ] Add `ruff` to `requirements-dev.txt` (new file): `ruff>=0.4`
- [ ] Document CI status badge in `README.md`: `[![CI](https://github.com/<org>/<repo>/actions/workflows/ci.yml/badge.svg)](…)`
- [ ] Verify: push to a feature branch triggers all 3 CI jobs; `lint` and `unit-tests` pass; `docker-build` completes without error
